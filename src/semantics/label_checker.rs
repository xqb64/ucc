...
pub trait LabelCollect {
    fn collect_labels(&mut self, labels: &mut HashSet<String>, funcname: &str) -> Result<&mut Self>;
}
...
impl LabelCollect for BreakStatement {
    fn collect_labels(&mut self, _labels: &mut HashSet<String>, funcname: &str) -> Result<&mut Self> {
        Ok(self)
    }
}
...
impl LabelCollect for LabeledStatement {
    fn collect_labels(&mut self, labels: &mut HashSet<String>, funcname: &str) -> Result<&mut Self> {
        if !labels.insert(format!("{}.{}", self.label.clone(), funcname)) {
            println!("inserted: {:?}", self.label.clone());
    ...
    fn collect_labels(&mut self, labels: &mut HashSet<String>, funcname: &str) -> Result<&mut Self> {
        match self {
            ...
            Declaration::Function(f) => {
                f.collect_labels(&mut HashSet::new(), &f.name.clone())?;
                Ok(self)
            }
...
impl LabelCheck for BreakStatement {
    fn label_check(&mut self, _labels: &mut HashSet<String>, funcname: &str) -> Result<&mut Self> {
        Ok(self)
    }
}
...
impl LabelCheck for GotoStatement {
    fn label_check(&mut self, labels: &mut HashSet<String>, funcname: &str) -> Result<&mut Self> {
        if !labels.contains(&format!("{}.{}", self.label.clone(), funcname)) {
            bail!("non existing label: {}", format!("{}.{}", self.label.clone(), funcname));
        }
        Ok(self)
    }
}
    ...
    fn label_check(&mut self, labels: &mut HashSet<String>, funcname: &str) -> Result<&mut Self> {
        match self {
            ...
            Declaration::Function(f) => {
                ...
                println!("collected labels for function '{}': {:?}", f.name, l);
