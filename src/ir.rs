...
fn address_taken_analysis(instrs: &[IRInstruction]) -> Vec<IRValue> {
    let addr_taken = |instr: &IRInstruction| -> Option<IRValue> {
        match instr {
            IRInstruction::GetAddress { src, dst: _ } => Some(src.to_owned()),
            _ => None,
        }
    };
